...
if not all([LOGIN_ID, SENHA, TG_TOKEN, TG_CHAT_ID]):
    ...
    logger.error("LOGIN_ID: {}/{}".format("✅" if LOGIN_ID else "❌", "Configurado" if LOGIN_ID else "Faltando"))
    logger.error("SENHA: {}/{}".format("✅" if SENHA else "❌", "Configurado" if SENHA else "Faltando"))
    logger.error("TG_TOKEN: {}/{}".format("✅" if TG_TOKEN else "❌", "Configurado" if TG_TOKEN else "Faltando"))
    logger.error("TG_CHAT_ID: {}/{}".format("✅" if TG_CHAT_ID else "❌", "Configurado" if TG_CHAT_ID else "Faltando"))
    sys.exit(1)
...
try:
    ...
    logger.info("✅ Tesseract configurado: /usr/bin/tesseract")
...
class HealthHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, *args): 
        pass  # Suprime logs do servidor HTTP

def start_health_server():
    port = int(os.environ.get('PORT', 8080))
    try:
        server = HTTPServer(('0.0.0.0', port), HealthHandler)
        logger.info(f"🖥️ Servidor de saúde rodando na porta {port}")
        server.serve_forever()
    except Exception as e:
        logger.error(f"❌ Erro no servidor de saúde: {e}")
    ...
    for tentativa in range(max_tentativas):
        try:
            ...
            for config in configs:
                ...
                if len(clean_text) == 4:
                    logger.info(f"✅ Captcha resolvido: {clean_text}")
                    return clean_text
