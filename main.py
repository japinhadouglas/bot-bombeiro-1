...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        html = "<html><body><h1>Bot Ativo</h1><p>" + status_info['resultado'] + "</p></body></html>"
...
def get_csrf_token(soup):
    # Lógica simplificada para evitar erro de sintaxe com regex
    for inp in soup.find_all('input', {'type': 'hidden'}):
        name = inp.get('name', '')
        value = inp.get('value', '')
        # Verifica se o valor é '1' e o nome tem 32 caracteres hexadecimais
        if value == '1' and len(name) == 32:
            try:
                # Tenta converter para int base 16 para validar se é hex
                int(name, 16)
                return name, value
            except ValueError:
                continue
